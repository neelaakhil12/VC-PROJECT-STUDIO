  ...
  useEffect(() => {
    const loadContact = async () => {
      try {
        ...
        console.error('Failed to load contact info in floating widget:', err);
      }
    };
    loadContact();
  }, [pathname]);
        ...
        {showWaSelect && (
          <div className="absolute right-0 bottom-14 bg-black border border-gold/30 rounded-lg p-3 shadow-2xl flex flex-col gap-2 min-w-[200px] z-50 animate-fade-in text-white">
            <p className="text-[10px] uppercase tracking-wider text-gold font-poppins font-semibold mb-1 border-b border-grey/25 pb-1">Select WhatsApp Chat:</p>
             <a 
              href={`https://wa.me/${contactInfo.whatsapp1}?text=Hi%20VC%20Project%20Studio%2C%20I%20would%20like%20to%20book%20a%20consultation%20for%20my%20home%20interiors.`}
              target="_blank"
              rel="noreferrer"
            ...
            <a 
              href={`https://wa.me/${contactInfo.whatsapp2}?text=Hi%20VC%20Project%20Studio%2C%20I%20would%20like%20to%20book%20a%20consultation%20for%20my%20home%20interiors.`}
              target="_blank"
              rel="noreferrer"
              onClick={() => setShowWaSelect(false)}
              className="flex items-center gap-2 hover:bg-gold/10 p-2 rounded transition-colors text-xs font-semibold text-white/90 hover:text-white"
...
function AppContent({ isConsultationOpen, setIsConsultationOpen, splashDone }: AppContentProps) {
  const { pathname } = useLocation();
  ...
  return (
    <div className="flex flex-col min-h-screen overflow-x-hidden max-w-full bg-[#F8F8F8] text-[#000000] selection:bg-[#D4AF37] selection:text-black">
      ...
      {!isAdmin && <Navbar onOpenConsultation={() => setIsConsultationOpen(true)} />}
